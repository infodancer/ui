...
- **Sidebar drawer mode (#43).** `.app-sidebar-layout` gains an opt-in `--drawers` modifier: on narrow viewports the asides become off-canvas drawers behind `.app-sidebar-toggle` buttons instead of stacking into the content column. The mechanism is the native Popover API (`popover` attribute + `popovertarget` buttons) — hidden-until-toggled, light dismiss, ESC, `::backdrop`, no JavaScript. Wide viewports neutralize the popover UA styles so the asides render inline exactly as the default layout; browsers without Popover support fall back to stacking, with the toggles gated behind `@supports selector(:popover-open)`. Default (stacking) behavior is unchanged.
- **Image lightbox (#44)** — a small authored asset (`assets/js/lightbox.js`) plus `LightboxHead(staticBase)` that emits its `<script>` tag, the second occupant of the track.js "minimal, opt-in, cross-cutting mechanism" category. A dependency-free viewer on the native `<dialog>` element: mark up images as `<a href="full.jpg" data-lightbox="group"><img src="thumb.jpg" alt="caption"></a>` and links sharing a `data-lightbox` value form a gallery — prev/next buttons, arrow keys, and a position counter, with the caption from `data-lightbox-caption` else the image's alt. `showModal()` supplies ESC, focus containment, and the `::backdrop`; clicking outside the figure closes. One delegated click listener, so content swapped in after load (htmx) needs no re-initialization; modified clicks (ctrl/cmd/shift/middle) pass through to the browser. Without JS the links open the full image — progressive enhancement. Hugo consumers bundle `js/lightbox.js` via `resources.Get`; Go consumers call `LightboxHead`. No SRI (first-party, same-origin asset, same rationale as track.js). New `base.css`: `.app-lightbox` and its `-close`/`-prev`/`-next`/`-counter` chrome. Replaces per-site npm lightboxes (mjh's simplelightbox).
