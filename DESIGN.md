...
`NavData.Links` is a flat, always-visible list. `NavData.Items` is the richer form: a tree of `MenuItem`s, each carrying a visibility `Gate`, that supports dropdowns and icon affordances (the notification bell). The partial renders `Items` when present and falls back to `Links` otherwise, so a flat-nav consumer is unaffected and migrates when it wants the new capability. *Gating* is a **Go-consumer feature**: it depends on knowing the request's viewer, which the static Hugo sites don't have. The *structural* half — dropdown submenus — exists in both variants: the Hugo partial renders `.Site.Menus.main` entries that have children as the same `<details class="app-nav-dropdown">` markup the Go variant emits (one level deep; Hugo menus express nesting via `parent`). A dropdown parent with its own URL is repeated as the first entry of its menu, since a `<summary>` can't also be a link. The Hugo auth area renders only when the site configures `.Site.Params.user` or `.Site.Params.ui.sign_in_url` — a public static site that sets neither gets no auth affordance.
